function [Hstar,WP,gamma,obj] = multikernelLatefusionAlignmentclustering2(K,k,lambda,tau,Y)
...
%%    gamma = ones(numker,1)/(numker);
    ...
    lb = zeros(numker,1);
    ub = ones(numker,1);
    [gamma] = quadprog(2*lambda*M,A,[],[],ones(1,numker),1,lb,ub);
    ...
    obj(iter) = trace(Hstar'*RpHpwpnew)-lambda*gamma'*M*gamma;
