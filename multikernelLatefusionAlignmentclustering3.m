function [Hstar,WP,gamma,obj] = multikernelLatefusionAlignmentclustering3(K,k,lambda1,tau,Y,lambda2)
...
%%    gamma = ones(numker,1)/(numker);
    A = zeros(numker,1);
    for i=1:numker
        A(i,1) = trace(Hstar'*(HP(:,:,p)*WP(:,:,p)));
    end
 
    M = corelation(HP,WP);
    lb = zeros(numker,1);
    ub = ones(numker,1);
    [gamma] = quadprog(2*lambda1*M,A,[],[],ones(1,numker),1,lb,ub);
    ...
    obj(iter) = trace(Hstar'*RpHpwpnew)-lambda1*gamma'*M*gamma + trace(Hstar'*lambda2*H0);
